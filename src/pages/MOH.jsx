  ...
  try {
    if (collectionDate && !data?.savedToDMS) {
      ...
      const updatePromises = orderIds.map(orderId => 
        axios.put(`/api/orders/${orderId}/collection-date`, {
...
const fetchSavedForms = async () => {
  try {
    ...
    const response = await axios.get('/api/gr_dms/forms');
...
useEffect(() => {
  const fetchMohOrders = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/orders?role=moh', {
        headers: {
          'X-User-Role': 'gorush'
        }
      });
  ...
  const fetchSavedOrderIds = async () => {
    try {
      const response = await axios.get('/api/gr_dms/saved-orders');
    ...
    function exportToExcel() {
      try {
        if (window.opener && !window.opener.closed) {
          ...
          const fallbackUrl = '/api/export-packing-list?data=' + packingListData;
...
const reDownloadExcelFromForm = async (formId) => {
  try {
    const response = await axios.get(`/api/gr_dms/forms/${formId}`);
...
const handleSaveToDMS = async (batchValue = 1) => {
  ...
  try {
    ...
    const response = await axios.post('/api/gr_dms/forms', {
      formName: formName,
...
const calculateJobMethodStats = (orders) => {
 ...
 const getFilteredOrdersByTab = () => {
  ...
  const handleBulkCollectionUpdate = async () => {
    ...
    try {
      const response = await fetch('/api/orders/bulk-collection-date', {
        method: 'POST',
        headers: {
      ...
      const ordersResponse = await fetch('/api/orders?role=moh');
...
const handleViewSavedManifest = async (orderId) => {
  try {
    ...
    const response = await axios.get(`/api/gr_dms/forms/by-order/${orderId}`);
